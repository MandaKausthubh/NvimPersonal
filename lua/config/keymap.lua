...
-- create a new terminal with conda environment
vim.api.nvim_create_user_command("NewCondaTerminal", function()
  local env_name = vim.fn.input("Enter Conda environment name: ")
  if env_name == "" then
    print("No environment selected. Opening default shell.")
    vim.cmd("split | terminal")
  else
    vim.cmd("split | terminal")
    vim.fn.chansend(vim.b.terminal_job_id, "conda activate " .. env_name .. " && clear\n")
  end
end, {})
...
-- local function new_terminal_python()
--   local handle = io.popen("conda info --envs | grep '*' | awk '{print $1}'")
--   local env_name = handle:read("*a"):gsub("%s+$", "")
--   handle:close()
--
--   if env_name == "" then
--     print("No Conda environment is currently activated. Opening Python in default shell.")
--     vim.cmd("vsplit | terminal")
--     vim.fn.chansend(vim.b.terminal_job_id, "python\n")
--   else
--     print("Current Conda environment: " .. env_name)
--     vim.cmd("vsplit | terminal")
--     vim.fn.chansend(vim.b.terminal_job_id, "conda activate " .. env_name .. " && python\n")
--   end
-- end
...
--
--   if env_name == "" then
--     print("No Conda environment is currently activated. Opening IPython in default shell.")
--     vim.cmd("vsplit | terminal")
--     vim.fn.chansend(vim.b.terminal_job_id, "ipython --no-confirm-exit\n")
--   else
--     print("Current Conda environment: " .. env_name)
--     vim.cmd("vsplit | terminal")
--     vim.fn.chansend(vim.b.terminal_job_id, "ipython --no-confirm-exit\n")
...
wk.add({
  {
    ...
    { "<leader>tm", "<cmd>0r ~/templates/CompetitiveProgramming.cpp<CR>", desc = "template for quarto" },
    { "<leader>tq", "<cmd>0r ~/templates/MachineLearning.qmd<CR>", desc = "template for CP" },
  }
}, { mode = 'n'})
